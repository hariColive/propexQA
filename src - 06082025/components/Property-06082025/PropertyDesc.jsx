...
import Approved from '../../assets/images/property/approved.png';
...
const PropertyDesc = ({ property }) => {
 ...
 const getShortDescription = (text, maxChars = 250) => {
  ...
  return (
    ...
    <div className='RERABlock border-solid border-[1px] border-[#ccc] p-4 mt-4'>
     <div className='flex gap-2'>  <h5 className='font-bold'>RERA Approved</h5><img src={Approved} alt="RERA Approved" className='w-5 h-5' /></div>
     <h3 className='text-md mt-4'>RERA Id</h3>
     <p className='text-[#F8049C]'>{property.RERANumber}</p>
    </div>
    ...
    <div className='price-list-details p-4 shadow-xl'>
  <table className='min-w-full table-auto border-collapse'>
    <thead>
      <tr>
        <th className='px-4 py-2 text-center font-bold border-b'>Category</th>
        <th className='px-4 py-2 text-center font-bold border-b'>Area (sq. ft)</th>
        <th className='px-4 py-2 text-center font-bold border-b'>Price (Rs.)</th>
      </tr>
    </thead>
    <tbody>
      {Array.isArray(property.PriceList) && property.PriceList.length > 0 ? (
        property.PriceList.map((item, idx) => (
          <tr key={idx}>
            <td className='px-4 py-2 text-center'>{item.PropertyCategory}</td>
            <td className='px-4 py-2 text-center'>{item.SquareFeet}</td>
            <td className='px-4 py-2 text-center'>₹ {item.PricePerMonth}</td>
          </tr>
        ))
      ) : (
        <tr>
          <td className='px-4 py-2 text-center'>{property.PropertyCategory}</td>
          <td className='px-4 py-2 text-center'>{property.SquareFeet}</td>
          <td className='px-4 py-2 text-center'>₹ {property.PricePerMonth}</td>
        </tr>
      )}
    </tbody>
  </table>
